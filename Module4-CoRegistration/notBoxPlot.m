...
params.addParameter('subColors', false, @(x) islogical(x));
params.addParameter('nSubjects', 1, @(x) isnumeric(x) && isscalar(x));
params.addParameter('sdPatchColor', [0.6,0.6,1] , @(x) isnumeric(x) && length(x)==3);
params.addParameter('semPatchColor', [1,0.6,0.6] , @(x) isnumeric(x) && length(x)==3);
...
SUBCOLORS  = params.Results.subColors;
nSubjects  = params.Results.nSubjects;
sdPatchColor  = params.Results.sdPatchColor;
semPatchColor  = params.Results.semPatchColor;
...
for ii=1:length(uX)
    f=b==ii;
    [hTemp,statsTemp]=myPlotter(x(f),y(:,f),SUBCOLORS,nSubjects,sdPatchColor,semPatchColor);
    H = [H,hTemp];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h,statsOut]=myPlotter(X,Y,SUBCOLORS,nSubjects,sdPatchColor,semPatchColor)
    ...
    for k=1:length(X)
        ...
        if strcmp(style,'patch') 
            h(k).sdPtch=patchMaker(SD(k),sdPatchColor); % default color [0.6,0.6,1]
        end
        
        %Build patch surfaces for SEM 
        if strcmp(style,'patch') || strcmp(style,'sdline')
            h(k).semPtch=patchMaker(SEM(k),semPatchColor); % default color [1,0.6,0.6]
        end
        ...
        h(k).data=plot(thisX, thisY, '.', 'color', C,...
                       'markerfacecolor', C);
        
        %Build patch surfaces for the means, and optionally the medians
        if strcmp(style,'patch') || strcmp(style,'sdline')
       ...
       if SUBCOLORS
           clrmap = hsv(nSubjects);
           if length(thisY)>nSubjects
               clrmap = repmat(clrmap,(length(thisY)/nSubjects),1);
           end
           for cc = 1:length(thisY)
               plot(thisX(cc),thisY(cc),'o','color',clrmap(cc,:),'markerfacecolor',clrmap(cc,:))
           end
       end
    end
