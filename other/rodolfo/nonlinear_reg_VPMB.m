function nonlinear_reg_VPMB(par, repeat)
...
for r = 1:nR
    ...
    for d = 1:length(def)
        ...
        func012invt1_down = [ pw, 'func012invt1_down_warped_', def(d).type, '.nii.gz' ];
        ...
        if ~exist(func012invt1_down) || repeat(2)
            ...
            cmd = strcat(sprintf('antsRegistration --dimensionality 3 --float 0'), ...
                ...
                sprintf(' --output [%s/func012invt1_down_%s_,%s/func012invt1_down_warped_%s.nii.gz,%s/func012invt1_down_inv_warped_%s.nii.gz]', ...
        ...
        nlwarp = [ pw, 'func012invt1_down_moderate_0Warp.nii.gz' ];
        ...
        for v = 1:nvol
            ...
            cmd = sprintf('antsApplyTransforms -d 3 -i %s -o %s -r %s -t %s -t %s -n Linear', ...
                pre_func, post_func, invt1brain_down, mat2ants, nlwarp);
            system(cmd); clear cmd;
