...
addpath('/home/alexandresayal/Documents/MATLAB/spm12')
...
base_dir = '/home/alexandresayal/GitRepos/vpmb/cognestic22_multimodal_dcm'; % Change this to yours
...
derpth = fullfile(base_dir, 'data','derivatives','GLM');
...
taskName = 'task-AA_tr-2500';
nscan = 156; %780, 520 ,390, 156
TR = 2.5; %0.5, 0.75, 1, 2.5

% All fits go in this directory
fits_dir = fullfile(base_dir, 'fits', 'batch_script', 'fmri', taskName);

mkdir(fullfile(fits_dir,'templates','DCMs'))
mkdir(fullfile(fits_dir,'templates','GCMs'))
...
subs = compose('%02g', [1,2,3,5,6,7,8,10,11,12,15,16,21,22,23]);
nsub   = numel(subs);
...
numworkers = 8; % Number of workers for distributed computing (depends on system)
if numworkers > 0
...
% STEP 1: Data
%---------------------------------------------------------------------------------------
ref_sub = 1; % eg subject 15
outdir = fullfile(derpth,subdir{ref_sub}, taskName);
...
for u = 1:3 % number of conditions of interest !!!!!!!!!!!!!!!!
...
DCM.b(:,:,1) = zeros(DCM.n,DCM.n); % Corresponding to 'Motion'
DCM.b(:,:,2) = [                   % Corresponding to 'Coherent'
%     V3a  hMT  SPL
    [  1    1    0  ];   % V3a
    [  1    1    1  ];   % hMT
    [  0    1    1  ];   % SPL  
];
DCM.b(:,:,3) = [                   % Corresponding to 'Incoherent'
%     V3a  hMT  SPL
    [  1    1    0  ];   % V3a
    [  1    1    1  ];   % hMT
    [  0    1    1  ];   % SPL  
...
for s = 1:nsub
    inputs{5}{s,1} = fullfile(derpth,subdir{s},taskName,'SPM.mat');
end
...
for r = 1:numel(ROI_names)
    files = {};
    for s = 1:nsub
        inputs{5+r}{s,1} = fullfile(derpth,subdir{s},taskName,sprintf('VOI_%s_1.mat',ROI_names{r}));
    end
end
...
%% Fetch explained variance data
data = spm_dcm_fmri_check(GCM,true);
expVar = zeros(nsub,1);

for ii=1:nsub
    expVar(ii) = data{ii}.diagnostics(1);
end

fprintf('---\nMean explained variance across subjects for the full model is:\nM   = %0.2f\nSEM = %0.2f\n---\n', ...
    mean(expVar), ...
    std(expVar)/sqrt(nsub) );
...
% Save model space
mkdir(fullfile(fits_dir, 'templates', 'GCMs', 'Families'))
...
for k=1:8
    ...
    axis square
    xticklabels({'V3a','hMT','SPL'})
    xlabel('from')
    yticklabels({'V3a','hMT','SPL'})
    ylabel('to')
end
