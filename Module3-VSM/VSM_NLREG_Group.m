...
%% Packages
addpath('/SCRATCH/software/toolboxes/jsonlab/') % For dealing with json files
...
for ss = 1:length(subjectList)
    ...
    % find fieldmap correspondence
    % this is necessary because some fieldmaps are applied to more than one
    % functional run and hence they loose the name of the task in the
    % filename
    fmapFolder = fullfile(bidsFolder,'derivatives','fmriprep',subjectID,'fmap');
    D = dir(fullfile(fmapFolder,'*desc-preproc_fieldmap.json'));
    fmapList = cell(nTasks,1); % same structure as taskList
    
    for ff = 1:length(D)
        
       dat = loadjson(fullfile(fmapFolder,D(ff).name)); % load json
       
       aux = cellfun(@(x) strsplit(x,'_'),dat.IntendedFor,'UniformOutput',false); % split intended for field
       
       aux = cellfun(@(x) strjoin(x(2:3),'_'),aux,'UniformOutput',false); % join task name
        
       [~,ia,~] =  intersect(taskList,aux,'stable'); % find for which tasks is this fieldmap intended
        
       [fmapList{ia}] = deal(D(ff).name(1:end-5)); % build cell array of correspondence
    ...
    for rr = 1:length(taskList)
        ...
        fmapName = fmapList{rr};
        
        calculateVSM(bidsFolder,vsmFolder,subjectID,taskName,fmapName);
